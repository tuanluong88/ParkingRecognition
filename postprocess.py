...
class YoloPostProcessAnchorless:
    ...
    def __call__(self, x):
        # 1. 중첩된 튜플/리스트를 1차원 리스트(flat_list)로 평탄화하는 재귀 함수
        def flatten(item):
        ...
        # 입력 x를 완전히 평탄화하여 텐서들만의 1차원 리스트로 만듭니다.
        flat_tensors = flatten(x)
        
        # 2. 300개의 쿼리가 담긴 RT-DETR 최종 출력 텐서 찾기
        pred_boxes = None
        pred_scores = None
        pred_concat = None

        for xi in flat_tensors:
            # 안전하게 디바이스를 맞춥니다 (선택적)
            # xi = xi.cpu() 
        ...
        # 5. 각 쿼리에서 가장 높은 점수와 해당 클래스 라벨 가져오기
        max_scores, labels = torch.max(scores, dim=-1)

        # 6. 설정한 신뢰도(Confidence) 임계값으로 필터링
        ...
        # 7. Visualizer 요구 사항에 맞게 병합: [x1, y1, x2, y2, conf, cls] -> (N, 6) 형태
...
class RTDETRPostProcess:
    ...
    def __call__(self, x):
        # 1. 완벽한 평탄화 (재귀 함수)
        # 어떤 형태의 튜플/리스트가 중첩되어 들어와도 모두 분해해서 1차원 Tensor 리스트로 만듭니다.
        def flatten(item):
            if isinstance(item, (list, tuple)):
                flat = []
                for i in item:
        ...
        for xi in flat_tensors:
            # 에러를 일으켰던 문제의 예전 코드는 완전히 삭제되었습니다.
            # 객체 쿼리 차원(일반적으로 300)을 포함하는 텐서 탐색
            if hasattr(xi, 'shape') and 300 in xi.shape:
                if xi.shape[-1] == self.nc + 4:
                    pred_concat = xi
                elif xi.shape[-2] == self.nc + 4:
                    pred_concat = xi.transpose(-1, -2)
        ...
        if pred_concat is not None:
            ...
            shapes = [getattr(xi, 'shape', 'Unknown') for xi in flat_tensors]
            raise ValueError(f"RT-DETR 디코더 출력을 찾을 수 없습니다 (300 쿼리 예상). "
                             f"현재 추출된 텐서 모양들: {shapes}")
        ...
        # 5. 각 쿼리에서 가장 높은 점수와 해당 클래스 라벨 가져오기
        # RT-DETR decoder는 raw logit을 출력하므로 sigmoid로 [0, 1] 확률값으로 변환
        ...
        # 7. NMS로 중복 박스 제거 (양자화로 깨진 one-to-one 보정)
        keep = torchvision.ops.batched_nms(
            boxes_xyxy, max_scores, labels, iou_threshold=0.7
        )
        boxes_xyxy = boxes_xyxy[keep]
        max_scores = max_scores[keep]
        labels = labels[keep]

        # 8. 병합: [x1, y1, x2, y2, conf, cls] -> (N, 6)
        result = torch.cat(
            [boxes_xyxy, max_scores.unsqueeze(-1), labels.float().unsqueeze(-1)],
            dim=-1,
        )
