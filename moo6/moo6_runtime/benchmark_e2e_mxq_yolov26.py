...
def preprocess_yolo(img_path: str, img_size=(640, 640)):
    ...
    # 4. 정규화(/255.0)를 생략하고 uint8 상태를 유지
    # 5. [H, W, C] -> [1, C, H, W] 순서 및 차원 확장
    img = np.expand_dims(np.transpose(img, [2, 0, 1]), 0)
    
    # 6. 데이터 타입을 uint8로 확실히 지정하여 반환
    return img.astype(np.uint8)

if __name__ == "__main__":
    parser = ArgumentParser(description="Measure End-to-End inference time of YOLOv26 MXQ model")
    ...
    # YOLOv26 후처리 모듈 초기화
    postprocess = Yolo26SegPostProcessE2E(conf_thres=args.conf_thres, iou_thres=args.nms_iou_thres)
    ...
    print(f"총 {len(image_paths)}장 이미지에 대한 YOLOv26 End-to-End 속도 테스트를 시작합니다...\n")
    ...
    for i, img_path in enumerate(image_paths):
        ...
        img_tensor = preprocess_yolo(img_path)
        ...
        # 3. 후처리 (Post-processing) - Boxes와 Masks 디코딩 및 NMS
    ...
    print("\n" + "="*50)
    print("   YOLOv26 End-to-End 속도 테스트 결과 요약 (Warm-up 제외)   ")
    print("="*50)
