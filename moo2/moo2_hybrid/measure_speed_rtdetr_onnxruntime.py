...
# ⭐ 수정됨: 기존 HybridRTDETR 대신 ONNX Runtime 버전인 HybridRTDETR_ORT를 임포트합니다.
from inference_mxq_onnxruntime import HybridRTDETR_ORT
...
if __name__ == "__main__":
    parser = ArgumentParser(description="Measure End-to-End inference time of ORT Hybrid RT-DETR")
    parser.add_argument("--mxq-path", type=str, required=True, help="Path to the compiled MXQ model")
    # ⭐ 수정됨: pt-path 대신 decoder-path로 변경하여 ONNX 파일을 받습니다.
    parser.add_argument("--decoder-path", type=str, required=True, help="Path to the ONNX decoder model")
    ...
    print("시스템을 초기화합니다...")
    # ⭐ 수정됨: HybridRTDETR_ORT 객체를 생성합니다.
    ...
    for i, img_path in enumerate(image_paths):
        ...
        # [Step 2] 모델 추론 (Inference: NPU 인코더 + ORT 디코더)
    ...
    print("\n" + "="*55)
    print("      End-to-End 속도 테스트 결과 요약 (ORT 최적화 적용)      ")
    print("="*55)
    ...
    if len(total_times) > 1:
        ...
        print(f"  ├─ 평균 하이브리드 추론   : {avg_infer:.2f} ms (NPU + ORT)")
