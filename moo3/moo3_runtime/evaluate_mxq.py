...
def evaluate_predictions(predictions, gt_boxes, iou_threshold=0.5):
    ...
    # Assuming prediction format is: [x1, y1, x2, y2, confidence, class_id]
...
if __name__ == "__main__":
    ...
    parser.add_argument("--eval-iou-thres", type=float, default=0.50, help="IoU threshold for considering a detection as True Positive")
    ...
    total_FP = 0
    total_FN = 0
    total_instances = 0  # [추가됨] 전체 인스턴스(객체) 개수를 저장할 변수 초기화
    ...
    for img_path in image_paths:
        ...
        # Assume result is a list/array of detections [x1, y1, x2, y2, conf, cls] for the image
        # If your postprocess returns a nested list or dict, adjust this accordingly
        raw_result = postprocess(outputs)

        # 배치(Batch) 중 첫 번째 이미지의 결과를 가져오고, PyTorch Tensor를 List로 변환합니다.
        # 결과가 None(검출된 객체 없음)일 경우 빈 리스트를 반환하도록 처리합니다.
        ...
        # [추가됨] 현재 이미지에 있는 정답 객체(Instance)의 수를 전체 인스턴스 수에 누적
        total_instances += len(gt_boxes)
        
        # Calculate metrics for this image
        TP, FP, FN = evaluate_predictions(preds, gt_boxes, args.eval_iou_thres)
    ...
    print("\n--- Performance Results ---")
    print(f"Total Instances (GT) : {total_instances}")  # [추가됨] 전체 인스턴스 개수 출력
