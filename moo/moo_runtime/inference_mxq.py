...
def preprocess_yolo(img_input, img_size=(640, 640)):
    # 1. 입력값이 문자열(경로)인지 이미지 데이터(Numpy 배열)인지 확인
    if isinstance(img_input, str):
        # 경로일 경우 파일을 읽어옴
        img = cv2.imread(img_input, cv2.IMREAD_COLOR)
        if img is None:
            raise FileNotFoundError(f"이미지 파일을 찾을 수 없습니다: {img_input}")
    else:
        # 이미 이미지 데이터(배열)인 경우 그대로 사용
        img = img_input
