...
if __name__ == "__main__":
    ...
    # inference_mxq.py 에 다음 코드를 추가하여 실행해 보세요.
    print("=== MXQ 모델 출력 텐서 정보 ===")
    for i, out in enumerate(outputs):
        print(f"Output {i}: shape={out.shape}, dtype={out.dtype}")
    print("================================")
    
    # 결과 후처리 (NMS 없음)
    results = postprocess(outputs, img_shape=(640, 640))
    det, masks = results[0]
    
    # 결과 시각화
    visualizer.draw(args.image_path, [det], masks=[masks], output_path=args.output_path)
    print(f"추론 완료! 결과가 {args.output_path} 에 저장되었습니다.")